...
3. 변경 표면 선택:
   ...
   - 레포에 남겨야 하면 `vox agent init`, `vox agent add`, 또는 dashboard export 파일의 `vox agent import`로 source 생성 후 `vox agent set` 또는 `agents/<name>/agent.json` 편집으로 설정을 바꾸고 `vox doctor` 또는 `vox agent doctor` 후 `vox agent validate/diff/push` 실행. 프로덕션 승격은 사용자가 승인한 경우에만 `vox agent version save`와 `vox agent promote --yes`로 이어간다.
