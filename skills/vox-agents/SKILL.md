...
7. **변경 표면 선택** — 조회·스키마 확인·one-off 생성은 MCP가 적합하다. 그러나 사용자가 레포, 코드, diff, 리뷰, PR, 롤백, CI, 커밋, 또는 "agent-as-code" 맥락을 언급하면 `vox` CLI 루프로 변경한다. 코딩 에이전트가 durable 변경을 해야 할 때 기본 경로는 `vox agent pull/init/add/import -> agent set 또는 edit agents/<name>/agent.json -> doctor/validate -> diff/status -> push` 이다.
