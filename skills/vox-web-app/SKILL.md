...
- **레포에 남길 agent/tool/knowledge 변경** → `using-vox-skills` 라우터를 거쳐 `vox` CLI Agent-as-Code 루프(`pull/init -> edit files -> doctor/validate -> agent status --all --offline -> diff/status -> push`, 필요 시 `version save -> promote`)로 넘긴다. 이 스킬은 화면 조작 안내와 UI-only 기능을 맡는다.
