...
4. 확인 받으면 변경 표면을 고른다.
   ...
   - 레포에 남겨야 하면 `vox agent init` 또는 `vox agent add`로 local source를 만들고 `vox doctor` 또는 `vox agent doctor` 후 `vox agent validate/diff/push` 루프를 사용하도록 `vox-agents`에 handoff
   - name: 이름
