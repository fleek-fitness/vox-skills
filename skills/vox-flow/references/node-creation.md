...
7. 레포에 남길 변경이면 CLI 경로를 우선한다: `agents/<name>/agent.json` 또는 `vox agent flow ...` helper로 수정 → `vox doctor` 또는 `vox agent doctor` → `vox agent validate` → `vox agent diff` → `vox agent push`.
