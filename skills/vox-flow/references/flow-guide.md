...
4. `vox doctor` 또는 `vox agent doctor`, `vox agent validate`, `vox agent status --all --offline`, `vox agent diff` 로 확인한 뒤 `vox agent push` 한다.
...
6. `vox agent diff/status` 또는 `get_agent` 로 다시 읽어 unknown field drop, enum mismatch, 누락 edge 를 확인한다. 여러 agent가 있는 CLI 프로젝트에서는 `vox agent status --all --offline`으로 project-wide 상태를 먼저 훑는다.
